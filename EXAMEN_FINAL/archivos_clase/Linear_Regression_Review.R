...
plot(mtcars$wt, mtcars$mpg, main="Diagrama de dispersión", xlab="Peso del vehículo", ylab="Millas por galón", pch=19)

###Coeficiente de correlación de Pearson (Individuales y data frame)
...
###Boxplot para detección temprana de outliers
par(mfrow=c(1, 2))  # divide área de gráfico en 2 columnas
...
###Gráficos de densidad para análisis preliminares distribucionales
###install.packages("e1071")
library(e1071)
par(mfrow=c(1, 2))  # divide graph area in 2 columns
plot(density(cars$speed), main="Gráfico de densidad: Velocidad", ylab="Frecuencia", sub=paste("Asimetría:", round(e1071::skewness(cars$speed), 2)))  # Gráfico de densidad para 'speed'
polygon(density(cars$speed), col="red")
plot(density(cars$dist), main="Gráfico de densidad: Distancia", ylab="Frecuencia", sub=paste("Asimetría:", round(e1071::skewness(cars$dist), 2)))  # Gráfico de densidad para 'dist'
...
###Análisis residual por wallyplot
...
help(faithful)
head(faithful)
###TAREA: No olvidar hacer todos los análisis complementarios(descriptivo, correlación, verificación de supuestos etc.)
eruption.lm = lm(eruptions ~ waiting, data=faithful) 
summary(eruption.lm) 
###Creación de intervalos de confianza para parámetros
confint(eruption.lm)
###Review A.3.2 Coeficiente de determinación
names(summary(eruption.lm))
summary(eruption.lm)$r.squared
###Review A.3.3 Error estándar residual, raíz del CME
...
###Review A.4
### Método "stepwise" para modelos de regresión lineal múltiple
...
step(modelo.completo, data=prostata, direction="backward",trace=FALSE)  ### Backward: Eliminación un paso atrás
step(modelo.nulo, scope=list(lower=modelo.nulo, upper=modelo.completo),data=prostata, direction="forward",trace=FALSE) ### Inclusión un paso adelante
step(modelo.completo, data=prostata, direction="both",trace=FALSE) ### Stepwise: Paso a paso - Combinación de Backward y Forward
...
step(completo.casas, data=X, direction="backward",trace=FALSE)  ### Backward: Eliminación un paso atrás
...
###A.6.3 Intervalos de predicción
...
# Error de validació  cruzada para regesión simple
set.seed(123)
juice.lm <- train(y ~ poly(x1, 20), data=df, method="lm", trControl=controlConfig)
# Error de validació  cruzada para regesión polinomial
...
###Validación cruzada en modelos "precios de casas"
...
###Regresión con predictores binarios
...
###
###Diagnósticos del modelo lineal - Temas adicionales (Su et al. 2012)
###

##########################################################################
# 				Autopartes							 #
#												 #
# El gerente de ventas de una empresa dedicada a la comercialización de  #
# autopartes desea desarrollar un método para pronosticar las ventas     #
# anuales de una región. En apariencia varios factores están relacionados#
# con las ventas y de acuerdo con la opinión del gerente las variables   #
# que deben intervenir en el análisis son 					 #
# 												 #
#      Y = Ventas anuales (en millones de dólares)				 #
#	 X_1 = Número de tiendas de venta al menudeo				 #
#      X_2 = Número de autos registrados (en millones)			 #
#      X_3 = Ingreso personal (en millones de dólares)			 #
# 	 X_4 = Antiguedad promedio de los automóviles (en años)		 #
# 	 X_5 = Número de supervisores							 #
#												 #	
# La información correspondiente a diez regiones elegidas al azar se     #
# encuentra en el archivo Autopartes.						 #	
# Los diagnósticos del modelo de regresión lineal múltiple               #
# estimado se muestran a continuación.						 #
#												 #
##########################################################################
...
###################################################
# Verificación de los supuestos sobre los errores #
###################################################

#Prueba de normalidad, H0: Los errores siguen una distribución normal
...
#Pueba para autocorrelación, H0: No hay autocorrelación, es decir los errores son independientes
library(lmtest)
dwtest(fit)
...
###Herramienta diagnóstica que trata cada variable independiente de la misma forma
...
###DFFits
###Si |Dffitsi|>1 Posible influyente (conjuntos de datos pequeños y medianos)
...
###Análisis gráfico del paquete OLSRR
