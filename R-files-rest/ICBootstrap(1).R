...
# Distribución de una Gamma (1,1/2)
x <- seq(0,8,length=16)
fx<- dgamma(x,1,rate=1/2)
plot(x,fx,type="l",main="Función de densidad Gamma(1,1/2)",xlab="X",ylab="Densidad",col="blue")
abline(v=2,lty=2)
#linex<-rep(2,11)
...
# Muestra de tamaño 16 de una Gamma (1,1/2)
...
# Distribución muestral teórica de Xbar
x <- seq(0,8,length=100)
fxc<- dgamma(x,16,rate=8)
plot(x,fxc,type="l",main="Distribución muestral Gamma(16,8)",xlab="Xbar",ylab="Densidad",col="blue")
abline(v=2,lty=2)

# Distribución bootstrap para las Xbar de muestras de tamaño 16

# Selecciona una muestra de tamaño 16 de una Gamma(1,1/2)
my.sample <- rgamma(16, 1, 1/2)

# Simula la distribución bootstrap basada en 10^5 (10000) remuestras
...
####################################################
#								   #
# Estimación de la distribución - Ejemplo Arsénico #
#								   #
####################################################
...
# Histograma y QQPlot de los niveles de arsénico en 271 pozos de Bangladesh
hist(Arsenic) 
qqnorm(Arsenic)
...
# Histograma y QQPlot de la distribución botstrap de la concentración 
# media de arsénico
...
# La distribución bootstrap parece bastante normal con un poco de asimetría
# Teorema Central del Límite "en acción" cuando el tamaño de muestra es lo
# suficientemente grande. La distribución muestral para la media es aprox.
# normal aún si la población no es normal.

mean(arsenic.mean) # media bootstrap, similar a la media muestral xbar=125.31 
# [1] 125.5375
mean(arsenic.mean)-mean(Arsenic) # bias - sesgo
# [1] 0.2175773
sd(arsenic.mean) # error estándar bootstrap - EE (SE)
# [1] 18.25759

# Cálculo de los puntos que están 1.96 EE a partir de la media de la
# distribución bootstrap
...
# Para la distribución normal, se sabe que los percentiles 0.025 y 0.975
# están en la media ±1.96 desviaciones estándar.
# Para esta distribución bootstrap en particular se encontró que
...
# En este caso el TCL será inexacto.
...
#####################################################
#								    #
# Ejemplo. Skateboard - Comparación de dos muestras #
#								    #
#####################################################
...
hist(TestMean, main = "Distribución bootstrap de la diferencia de medias",
     xlab="Dif. Medias")
...
# hombres patinadores frente a una mujer están en promedio 
# entre 24.42 y 139.12 ng/dl más altos que los niveles frente a un hombre.
...
#######################################################
#									#
# Prueba de permutación para las medias de Skateboard #
#									#
#######################################################	
...
N <- 10^4 - 1  # número de veces que se repite el proceso

#set.seed(99) 
result <- numeric(N) # espacio para salvar las diferencias aleatorias
for(i in 1:N)
  {
  index <- sample(71, size = nf, replace = FALSE) # muestra de números entre 1:71
...
main="Distribución de permutación para niveles de testosterona")
